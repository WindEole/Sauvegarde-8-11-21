/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   algorithme.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: iderighe <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/08/10 12:24:06 by iderighe          #+#    #+#             */
/*   Updated: 2021/10/05 10:56:49 by iderighe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

void	ft_algo_2_3(int *s_a, t_varlen *varlen)
{
	if (varlen->len_a == 2)
	{
		if (s_a[0] > s_a[1])
			ft_swap_a(s_a, varlen);
		else
			return ;
	}
	else
		ft_algo_3(s_a, varlen);
	return ;
}

void	ft_algo_3(int *s_a, t_varlen *varlen)
{
	if (s_a[0] < s_a[1])
	{
		ft_revrotate_a(s_a, varlen);
		if (s_a[0] > s_a[1])
			ft_swap_a(s_a, varlen);
		else
			return ;
	}
	if (s_a[0] > s_a[1])
	{
		if (s_a[0] < s_a[2])
			ft_swap_a(s_a, varlen);
		else
		{
			ft_rotate_a(s_a, varlen);
			if (s_a[0] > s_a[1])
				ft_swap_a(s_a, varlen);
		}
		return ;
	}
	return ;
}

void	ft_algo_common_4_5(int *s_a, int *s_b, t_varlen *varlen)
{
	if (s_b[0] < s_a[0])
		ft_push_a(s_a, s_b, varlen);
	else if (s_b[0] > s_a[varlen->len_a - 1])
	{
		ft_push_a(s_a, s_b, varlen);
		ft_rotate_a(s_a, varlen);
	}
	else if (s_b[0] > s_a[0] && s_b[0] < s_a[1])
	{
		ft_rotate_a(s_a, varlen);
		ft_push_a(s_a, s_b, varlen);
		ft_revrotate_a(s_a, varlen);
	}
	return ;
}

void	ft_algo_4_5(int *s_a, int *s_b, t_varlen *varlen)
{
	if (varlen->len_a == 4)
	{
		ft_push_b(s_a, s_b, varlen);
		if (!((s_a[0] < s_a[1]) && (s_a[1] < s_a[2])))
			ft_algo_3(s_a, varlen);
		if (s_b[0] < s_a[0] || s_b[0] > s_a[varlen->len_a - 1]
			|| (s_b[0] > s_a[0] && s_b[0] < s_a[1]))
			ft_algo_common_4_5(s_a, s_b, varlen);
		else if (s_b[0] > s_a[0] && s_b[0] > s_a[1])
		{
			ft_revrotate_a(s_a, varlen);
			ft_push_a(s_a, s_b, varlen);
			ft_rotate_a(s_a, varlen);
			ft_rotate_a(s_a, varlen);
		}
		return ;
	}
	else
		ft_algo_5(s_a, s_b, varlen);
	return ;
}

void	ft_algo_5(int *s_a, int *s_b, t_varlen *varlen)
{
	if (s_a[0] > s_a[1])
		ft_swap_a(s_a, varlen);
	ft_push_b(s_a, s_b, varlen);
	ft_algo_4_5(s_a, s_b, varlen);
	if (s_b[0] < s_a[0] || s_b[0] > s_a[varlen->len_a - 1]
		|| (s_b[0] > s_a[0] && s_b[0] < s_a[1]))
		ft_algo_common_4_5(s_a, s_b, varlen);
	else if (s_b[0] > s_a[0] && s_b[0] > s_a[1] && s_b[0] > s_a[2])
	{
		ft_revrotate_a(s_a, varlen);
		ft_push_a(s_a, s_b, varlen);
		ft_rotate_a(s_a, varlen);
		ft_rotate_a(s_a, varlen);
	}
	else if (s_b[0] > s_a[0] && s_b[0] > s_a[1] && s_b[0] < s_a[2])
	{
		ft_revrotate_a(s_a, varlen);
		ft_revrotate_a(s_a, varlen);
		ft_push_a(s_a, s_b, varlen);
		ft_revrotate_a(s_a, varlen);
		ft_revrotate_a(s_a, varlen);
	}
	return ;
}
